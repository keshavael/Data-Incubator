...
case1902
#Part a 
deathProp <- Death/(Death + NoDeath)
oddsDeath <- Death/(NoDeath +0.5)
shapePlot <- ifelse(Victim=="White",22,24)
colorPlot <- ifelse(Victim=="White","white","black")
plot(oddsDeath ~ Aggravation, log="y", pch=shapePlot, bg=colorPlot)
legend("bottomright", "(x,y)", pch = 1, title = "Dark for Black victime, square for white victims")
# fit logistic regression
logModelWithRace <- glm(cbind(Death,NoDeath) ~ Aggravation + Victim + 
                Aggravation:Victim, family=binomial)
summary(logModelWithRace)
logModelWithoutRace <- update(logModelWithRace, ~ . - Aggravation:Victim)
summary(logModelWithoutRace)
anova(logModelWithRace, logModelWithoutRace) # no evidence of interaction.

fitGoodness <- 1-pchisq(logModelWithoutRace$deviance,df=logModelWithoutRace$df.residual)
fitGoodness
resid1 <- resid(logModelWithoutRace$deviance, type="deviance")
cbind(Death,NoDeath, resid1)
plot(Death, resid1, ylab="Deviance residual")
